...
task("deploy", "Deploy Guardian verifier contract")
  ...
  .addOptionalParam(
    ...
    "UpdateGuardianVerifier contract address",
    undefined,
    types.string,
  )
  .addOptionalParam(
    "socialRecoveryVerifier",
    "SocialRecoveryVerifier contract address",
    undefined,
    types.string,
  )
  .addOptionalParam(
  ...
  .setAction(
    async (
      ...
      if (!updateGuardianVerifier) {
        const UpdateGuardianVerifier = await ethers.getContractFactory(
          "UpdateGuardianVerifier",
        )
        ...
        console.log(
          `deploy update Guardian verifier to testnet in ${updateGuardianVerifier}`,
        )
      }

      if (!socialRecoveryVerifier) {
        const SocialRecoveryVerifier = await ethers.getContractFactory(
          "SocialRecoveryVerifier",
        )
        ...
        console.log(
          `deploy social recovery verifier to testnet in ${socialRecoveryVerifier}`,
        )
      }
      ...
      if (!privateRecoveryAccountFactory) {
        ...
        const PrivateRecoveryAccountFactory = await ethers.getContractFactory(
          "PrivateRecoveryAccountFactory",
          {
            libraries: {
              "contracts/GuardianStorage.sol:GuardianStorage":
                guardianStorageLib.address,
            },
          },
        )
        const _privateRecoveryAccountFactory =
          await PrivateRecoveryAccountFactory.deploy(
        ...
        console.log(
          `deploy private recovery account factory to testnet in ${privateRecoveryAccountFactory}`,
        )
      }
      ...
      const addresses = {
        ...
        socialRecoveryVerifier,
        privateRecoveryAccountFactory,
        poseidon: poseidonT3Lib.address,
      }
      console.log(
        "🚀 ~ file: deploy.js:51 ~ .setAction ~ addresses:",
        addresses,
      )

      // get deployed instances
      const updateGuardianVerifierIns = await hre.ethers.getContractAt(
        "UpdateGuardianVerifier",
        updateGuardianVerifier,
      )
      const socialRecoveryVerifierIns = await hre.ethers.getContractAt(
        "SocialRecoveryVerifier",
        socialRecoveryVerifier,
      )
      const privateRecoveryAccountFactoryIns = await hre.ethers.getContractAt(
        "PrivateRecoveryAccountFactory",
        privateRecoveryAccountFactory,
      )
